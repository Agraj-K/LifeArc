...
export default function Goals() {
  ...
  const handleEdit = (goal) => {
    setFormData({ title: goal.title, category: goal.category, description: goal.description, targetDate: goal.targetDate?.split('T')[0] || '', progress: goal.progress, milestones: goal.milestones })
    setEditingId(goal._id); setIsCreating(true)
  }
    ...
    <div className="relative min-h-screen w-full overflow-hidden" style={{ backgroundColor: 'hsl(201, 100%, 13%)' }}>
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-8 pt-24 pb-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-12 animate-fade-rise">
          <div>
            <h1 className="text-5xl md:text-6xl text-white tracking-tight" style={{ fontFamily: "'Instrument Serif', serif" }}>Future <em className="not-italic text-white/40">Goals</em></h1>
            <p className="text-white/50 text-base mt-4">Set milestones, track progress, achieve what matters.</p>
          </div>
          <button onClick={() => { setIsCreating(true); setEditingId(null); setFormData({ title: '', category: 'Career', description: '', targetDate: '', progress: 0, milestones: [] }) }} className="liquid-glass rounded-full px-6 py-3 text-sm text-white cursor-pointer hover:scale-105 transition-transform flex items-center gap-2">
            ...
            <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-black/20 backdrop-blur-2xl shadow-2xl shadow-black/50">
              ...
              <div className="relative p-8">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl text-white" style={{ fontFamily: "'Instrument Serif', serif" }}>{editingId ? 'Edit Goal' : 'Create New Goal'}</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mb-4">
                      <input type="text" value={newMilestone} onChange={e => setNewMilestone(e.target.value)} onKeyPress={e => e.key === 'Enter' && (e.preventDefault(), handleAddMilestone())} className="flex-1 bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-white placeholder:text-white/30 focus:outline-none focus:border-teal-500/50 focus:bg-white/10 transition-all duration-300" placeholder="Add a milestone..." />
                      <button type="button" onClick={handleAddMilestone} className="px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white/60 hover:bg-white/10 hover:text-white transition-all duration-300">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 5v14M5 12h14"/></svg>
                      </button>
                    </div>
                    <div className="space-y-2">
                      {formData.milestones.map((m, i) => (
                        <div key={i} className="flex items-center gap-3 p-3 rounded-lg bg-white/5 border border-white/10">
                          ...
                          <button type="button" onClick={() => setFormData({...formData, milestones: formData.milestones.filter((_, idx) => idx !== i)})} className="text-white/20 hover:text-red-400 transition-colors">
                  ...
                  <div className="flex justify-end gap-4">
                    ...
                    <button type="submit" className="liquid-glass px-8 py-3 rounded-xl text-white font-medium hover:scale-[1.02] active:scale-[0.98] transition-all duration-300">{editingId ? 'Update Goal' : 'Create Goal'}</button>
                  </div>
                </form>
              </div>
            </div>
        ...
        {loading ? (
          <div className="text-center py-24 text-white/40">Loading goals...</div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {goals.map((goal, index) => (
              <div key={goal._id} className="group relative overflow-hidden rounded-3xl border border-white/10 bg-black/20 backdrop-blur-2xl shadow-2xl shadow-black/50 animate-fade-rise" style={{ animationDelay: `${index * 100}ms` }}>
                ...
                <div className="relative p-8">
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      <span className={`inline-block px-3 py-1 rounded-full text-xs border mb-3 ${goal.category === 'Career' ? 'text-blue-400 border-blue-500/30' : goal.category === 'Health' ? 'text-emerald-400 border-emerald-500/30' : goal.category === 'Learning' ? 'text-purple-400 border-purple-500/30' : goal.category === 'Finance' ? 'text-yellow-400 border-yellow-500/30' : 'text-white/40 border-white/10'}`}>{goal.category}</span>
                      <h3 className="text-2xl text-white group-hover:text-teal-200 transition-colors" style={{ fontFamily: "'Instrument Serif', serif" }}>{goal.title}</h3>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => handleEdit(goal)} className="p-2 rounded-lg bg-white/5 border border-white/10 text-white/40 hover:bg-white/10 hover:text-white transition-all duration-300">
                      ...
                      <button onClick={() => handleDelete(goal._id)} className="p-2 rounded-lg bg-white/5 border border-white/10 text-white/40 hover:bg-red-500/20 hover:text-red-400 hover:border-red-500/30 transition-all duration-300">
                  ...
                  <div className="space-y-2">
                    ...
                    {goal.milestones.map((milestone, i) => (
                      <button key={i} onClick={() => toggleMilestone(goal._id, i)} className="w-full flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 transition-all duration-300 text-left">
                        <div className={`w-5 h-5 rounded border flex items-center justify-center transition-all duration-300 ${milestone.done ? 'bg-teal-500/20 border-teal-500/50' : 'border-white/20 hover:border-teal-500/30'}`}>
                        ...
                        <span className={`text-sm transition-all duration-300 ${milestone.done ? 'text-white/40 line-through' : 'text-white/70'}`}>{milestone.text}</span>
                      </button>
                    ))}
                  </div>
                </div>
            ...
            {goals.length === 0 && !loading && (
              <div className="col-span-2 text-center py-24">
                <h3 className="text-2xl text-white/60 mb-3" style={{ fontFamily: "'Instrument Serif', serif" }}>No goals yet</h3>
                <p className="text-white/40 mb-8">Set your first goal and start tracking your progress.</p>
                <button onClick={() => setIsCreating(true)} className="liquid-glass rounded-full px-8 py-3 text-white hover:scale-105 transition-transform">Create First Goal</button>
              </div>
            )}
          </div>
        )}
